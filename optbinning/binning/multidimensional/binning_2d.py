...
def _check_parameters(name_x, name_y, dtype_x, dtype_y, prebinning_method,
    ...
    # nan and inf are Numbers and neither is < 0, so the range test is what
    # rejects them. Written as a comparison rather than math.isfinite so an
    # arbitrary-precision int cannot raise OverflowError here.
    if (not isinstance(time_limit, numbers.Number) or
            not -np.inf < time_limit < np.inf or time_limit < 0):
        raise ValueError("time_limit must be a finite non-negative value in "
...
class OptimalBinning2D(OptimalBinning):
    ...
    def _fit_optimizer(self, splits_x, splits_y, NE, E):
        ...
        if (t_n_event == 0) != (t_n_nonevent == 0):
            ...
            # No solver ran, so the estimator must carry no solver record:
            # a refit onto a degenerate target would otherwise leave
            # information() reporting the previous fit's objective, and
            # splitting the new solver time against the old model-generation
            # time gives a negative percentage.
            self._optimizer = None
            self._time_optimizer = None
            self._time_model_data = 0.
